...
export async function getAvailableModel(): Promise<string> {
  const isGemini = ENV.forgeApiUrl.includes("generativelanguage.googleapis.com") || !ENV.forgeApiKey.startsWith("sk-");
  ...
  try {
    ...
    // Preferences: gemini-1.5-flash, gemini-2.5-flash, gemini-3.5-flash, gemini-2.0-flash, gpt-4o-mini
    const preferences = ["gemini-1.5-flash", "gemini-2.5-flash", "gemini-3.5-flash", "gemini-2.0-flash", "gpt-4o-mini"];
    ...
    const fallback = response.data.find(m => m.id.startsWith("gemini") || m.id.startsWith("gpt") || m.id.startsWith("claude"))?.id;
    if (fallback) return fallback;

    if (ids.length > 0) return ids[0];
  } catch (error) {
    console.error(`[LLM] Failed to list models, defaulting to ${defaultModel}:`, error);
  }
  return defaultModel;
}
  ...
  if (isGemini) {
    ...
    if (!response.ok) {
      ...
      if (!fallbackResponse.ok) {
        throw new Error(`Gemini embedding failed: ${await fallbackResponse.text()}`);
      }
      const json = await fallbackResponse.json() as { embedding: { values: number[] } };
